...
-- Finisher以外のコスメ種別判定
local function isAllowedCosmetic(name, cType)
    if not name or name:find("MISSING_") then return false end
    if cType == "Finisher" or (name and name:lower():find("finisher")) then
        return false
    end
    return true
end
...
-- ==================== Cosmetic Library フック ====================
local function checkOwns(name)
    local cosmetic = CosmeticLibrary.Cosmetics[name]
    local cType = cosmetic and cosmetic.Type or ""
    return isAllowedCosmetic(name, cType)
end

CosmeticLibrary.OwnsCosmeticNormally = function(self, inventory, name, weapon) return checkOwns(name) end
CosmeticLibrary.OwnsCosmeticUniversally = function(self, inventory, name, weapon) return checkOwns(name) end
CosmeticLibrary.OwnsCosmeticForWeapon = function(self, inventory, name, weapon) return checkOwns(name) end

local originalOwnsCosmetic = CosmeticLibrary.OwnsCosmetic
CosmeticLibrary.OwnsCosmetic = function(self, inventory, name, weapon)
    if checkOwns(name) then return true end
    ...
    if key == "CosmeticInventory" then
        ...
        if data then
            for k, v in pairs(data) do
                if checkOwns(k) then proxy[k] = v end
            end
        end
        return setmetatable(proxy, {__index = function(t, k)
            if checkOwns(k) then return true end
            return nil
        end})
    end
    if key == "FavoritedCosmetics" then
        ...
        for weapon, favs in pairs(favorites) do
            ...
            for name, isFav in pairs(favs) do
                if checkOwns(name) then result[weapon][name] = isFav end
            end
        end
        return result
    end
    ...
    if equipRemote then
        ...
        oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            if self == equipRemote then
                ...
                if not isAllowedCosmetic(cosmeticName, cosmeticType) then return oldNamecall(self, ...) end
            ...
            if self == favoriteRemote then
                if isAllowedCosmetic(args[2], "") then
...
if ClientItem and ClientItem._CreateViewModel then
    ...
    ClientItem._CreateViewModel = function(self, viewmodelRef)
        ...
        if weaponPlayer == player and equipped[weaponName] and viewmodelRef then
            ...
            if targetData then
                for _, cType in ipairs({"Skin", "Charm", "Wrap"}) do
                    if equipped[weaponName][cType] then
                        targetData[self:ToEnum(cType) or cType] = equipped[weaponName][cType]
                        targetData[self:ToEnum("Name") or "Name"] = equipped[weaponName][cType].Name
                    end
                end
            end
        end
...
if viewModelModule then
    ...
    ClientViewModel.new = function(replicatedData, clientItem)
        ...
        if weaponPlayer == player and equipped[weaponName] then
            ...
            for _, cType in ipairs({"Skin", "Charm", "Wrap"}) do
                if equipped[weaponName][cType] then
                    replicatedData[dataKey][ReplicatedClass:ToEnum(cType)] = equipped[weaponName][cType]
                end
            end
        end
    ...
    if EmoteController and EmoteController.GetEmotes then
        ...
        EmoteController.GetEmotes = function(self)
            ...
            for name, cosmetic in pairs(CosmeticLibrary.Cosmetics) do
                if isAllowedCosmetic(name, cosmetic and cosmetic.Type or "") and (cosmetic.Type == "Dance" or cosmetic.Type == "Emote" or name:lower():find("dance") or name:lower():find("emote")) then
